			...
			success: function success(data) {
				...
				for (var i = 0; i < data.info.top.length; i++) {
					...
					if (i < 3) {
						...
						$('.rank_ul_one .li' + i).find('span').eq(0).find('img').attr('src', 'http://s.speaka.live/' + data.info.top[i].head);
				...
				$('.rank_ul_three li').find('span').eq(0).find('img').attr('src', 'http://s.speaka.live/' + data.info.mine.head);
